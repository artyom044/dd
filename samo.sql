--Люди (Useer) - которые катаются
--Самокаты (Scooter) - на которых катаются
--Тарифы (Tariff) - сколько стоит минута
--Промокоды (Promocode) - скидки
--Прокаты (Rental) - запись о поездке
--Платежи (Payment) - оплата
--История промокодов (PromoUsage) - кто какие скидки использовал
